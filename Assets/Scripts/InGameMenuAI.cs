...
public class InGameMenuAI : MonoBehaviour
{
    ...
    public void ToggleSettings()
    {
        ...
        if (!settingsOut)
        {
            MainMenuAI.MM.SettingsMenu.transform.DOMove(settingsEndpoint.position, .2f);
            jukeboxMenu.DOMove(jukeboxEndpoint.position, .2f);
            settingsOut = true;
        }
        else
        {
            MainMenuAI.MM.SettingsMenu.DOMove(MainMenuAI.MM.gotoPoint.position, .2f);
            jukeboxMenu.DOMove(MainMenuAI.MM.gotoPoint.position, .2f);
            settingsOut = false;
        }
    }
